...
import { getProfessionalById, updateProfessional, getLocations } from '@/lib/data';
...
import { PlusCircle, Edit2, Users, Search, Loader2, CalendarDays, Clock, Trash2, Calendar as CalendarIconLucide, AlertTriangle, Moon, ChevronsDown, FileText, Building, Gift, Briefcase as BriefcaseIcon, ChevronLeft, ChevronRight, Bed, DollarSign, Percent, Save, ArrowLeft, Layers } from 'lucide-react';
...
export default function EditProfessionalPage() {
    ...
    useEffect(() => {
        async function loadData() {
            setIsLoading(true);
            try {
                const [profData, locsData] = await Promise.all([
                ...
                DAYS_OF_WEEK.forEach(day => {
                    ...
                    const schedule = profData.workSchedule?.[dayId];
                ...
                form.reset({
                    ...
                    locationId: profData.locationId as LocationId,
                    phone: profData.phone || undefined, 
                    isManager: profData.isManager || false,
                    baseSalary: profData.baseSalary ?? 1025,
                    commissionRate: (profData.commissionRate ?? 0.20) * 100,
                    commissionDeductible: profData.commissionDeductible ?? (profData.locationId === 'higuereta' ? 2200 : 1800),
                    afp: profData.afp ?? 128.5,
                    seguro: profData.seguro ?? 101.7,
                    workSchedule: formWorkSchedule,
                    customScheduleOverrides: profData.customScheduleOverrides?.map(ov => ({
                        ...ov,
                        date: parseISO(ov.date),
                    })) || [],
                    currentContract_startDate: profData.currentContract?.startDate ? parseISO(profData.currentContract.startDate) : null,
                    currentContract_endDate: profData.currentContract?.endDate ? parseISO(profData.currentContract.endDate) : null,
                    currentContract_notes: profData.currentContract?.notes || undefined,
                    currentContract_empresa: profData.currentContract?.empresa || undefined,
                    birthDay: profData.birthDay ?? null,
                    birthMonth: profData.birthMonth ?? null,
                });

            } catch (err) {
                toast({ title: "Error", description: "No se pudieron cargar los datos del profesional.", variant: "destructive" });
            } finally {
                setIsLoading(false);
            }
        }
    ...
    const onSubmit = async (data: ProfessionalFormData) => {
        try {
            ...
            toast({ title: "Error", description: "No se pudo actualizar el profesional.", variant: "destructive" });
            console.error(error);
        }
    };
            ...
            <Card>
                <CardHeader>
                    <div className="flex justify-between items-center">
                        <div>
                            <CardTitle className="text-2xl">Editar Profesional</CardTitle>
                            <CardDescription>Modifique la información, horarios y contrato de {professional.firstName} {professional.lastName}.</CardDescription>
                        </div>
