  ...
  if (!patientId) {
    ...
    if (existingPatient) {
      ...
      const newPatient = await addPatient({
        ...
        // email: data.patientEmail, // Email removed from patient data
      ...
      if(existingPatientDetails) {
          ...
          // if(data.patientEmail !== existingPatientDetails.email) updatedPatientInfo.email = data.patientEmail; // Email removed
...
export const getPatientAppointmentHistory = async (patientId: string): Promise<Appointment[]> => {
  const allAppointments = await getAppointments({ patientId });
    ...
    .filter(a => {
      ...
      return apptDate < today && 
             (a.status === APPOINTMENT_STATUS.COMPLETED || 
              ...
              a.status === APPOINTMENT_STATUS.CANCELLED_CLIENT || 
              a.status === APPOINTMENT_STATUS.CANCELLED_STAFF);
    })
