...
const firebaseConfig = {
  ...
  projectId: "footprints-scheduler-ywrwg", // Hardcoded as per user request
...
// This should be determined by an environment variable
...
console.log("Firebase Config Module: Effective useMockDatabase:", useMockDatabase);
console.log("Firebase Config Module: Hardcoded firebaseConfig.projectId for initialization:", firebaseConfig.projectId);
...
if (useMockDatabase) {
  ...
  if (essentialConfigsMissing) {
    ...
    if (!firebaseConfig.projectId) console.error("Firebase Config Error: Missing NEXT_PUBLIC_FIREBASE_PROJECT_ID (Hardcoded but check .env if other issues persist). This is crucial for Firestore connection.");
    // Ensure app and firestoreInstance are explicitly undefined if config is missing
    app = undefined;
    firestoreInstance = undefined;
  } else {
    console.log("Firebase Config Module: All essential NEXT_PUBLIC_FIREBASE_ variables for REAL database connection appear to be present.");
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          ...
          if (firestoreInstance) {
            console.log("Firebase Config Module: Attempting to connect Firestore to emulator at localhost:8080 (if emulator is running).");
            try {
